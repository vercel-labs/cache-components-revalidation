...
Cached pages load fast, but the data behind them can change and become stale. For example, a product's price changes, a description gets updated, new inventory arrives. You need a way to refresh cached content without rebuilding and redeploying your entire app.
...
[`cacheLife`]() accepts a named profile that configures how long cached data stays fresh before revalidating in the background. If you don't specify a profile, Next.js uses `default`. The built-in profiles are:
...
The Route Handler is public, anyone who knows the URL can invalidate your cache. To prevent this, check for a secret token before processing the request.

Read the `Authorization` header and compare its bearer token against a server-side environment variable. If the token is missing or doesn't match, return a `401`:
...
Store `REVALIDATION_SECRET` as an environment variable on your hosting provider. For local development, add it to `.env`, and git ignore it.
